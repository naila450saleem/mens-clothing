...
const Navbar = () => {
  ...
  // Get subLinks for compact nav
  const categoryLinks = navLinks.find(link => link.name === "Our Categories")?.subLinks || [];

  return (
    <div className="page-bg-slider">
      
      {/* Compact navbar (only subLinks) */}
      {showCompactNav && (
      ...
      {/* Normal Navbar (hidden when scrolled) */}
      {!showCompactNav && (
        <>
          {/* Top Green Strip */}
            ...
            {navLinks.map(link => (
              <React.Fragment key={link.name}>
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `${link.subLinks ? 'has-submenu' : ''} ${isActive ? 'active' : ''}`
                  }
                >
                  {link.name}
                </NavLink>
